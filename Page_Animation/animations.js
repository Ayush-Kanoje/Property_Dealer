...
function prepareElements() {
  // Section headers
  document
    .querySelectorAll(".section_header, .section_title, h2, h3")
    .forEach((el) => {
  ...
  document
    .querySelectorAll(
      ".property_card, .feature_card, .testimonial_card, .carousel_item"
    )
    .forEach((el, index) => {
      el.classList.add("animate-scroll");
  ...
  if (animatedElements.length === 0) {
    document
      .querySelectorAll(
        "section, .section_header, .hero_content, .properties_section > div, .features_section > div"
      )
      .forEach((el) => {
        el.classList.add("animate-scroll");
        el.dataset.effect = "fade-up";
...
function setupScrollAnimations() {
  ...
  if (DEBUG)
    console.log(
      `Found ${animatedElements.length} elements to animate on scroll`
    );
    ...
    (entries) => {
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          if (DEBUG)
            console.log("Element is hidden, removing animation:", entry.target);
        }
      });
    },
    {
